...
try {
    ...
    if($RecordSet!=null) {
        ...
        while ($row = odbc_fetch_array($RecordSet)) {
            ...
            $sql = "";         
            if(!empty($row['clave'])) {
                $clave = $crypto->decryption($row['clave']);
...
$user = "corporativo";
$password = "c0rp0r@t1v0";
$dsn = "dbprueba";  // Este es el nombre de tu DSN configurado en el administrador ODBC
...
// Consulta SQL
$SQLQuery = "SELECT identificador, codigo, datos, empresa, estado FROM usuario_timbrado";
...
echo "<table border='1'>";
echo "<tr><th>Identificadorxx</th><th>Código</th><th>Datos</th><th>Empresa</th><th>Estado</th><th>Clave</th></tr>";
while ($row = odbc_fetch_array($RecordSet)) {
    echo "<tr>";
    echo "<td>" . $row['identificador'] . "</td>";
    echo "<td>" . $row['codigo'] . "</td>";
...
/*while (odbc_fetch_row($RecordSet)) {
    //echo "<pre>";print_r(odbc_result_all($RecordSet)); echo "</pre>";
    //echo "<pre>";print_r(odbc_result_all($RecordSet));echo"</pre>";
    odbc_result_all($RecordSet, "border=1");
}
